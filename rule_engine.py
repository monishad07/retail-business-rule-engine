...
# ------------------ RECOMMENDATIONS ------------------
def add_recommendation(alert):
    recommendations = {
        "HIGH_SALES_LOW_PROFIT": "Review pricing strategy and supplier costs.",
        "DISCOUNT_RISK": "Reduce discount levels or introduce bundled offers.",
        "PRODUCT_PROFIT_DECLINE": "Analyze demand trends and competitor pricing.",
        "REGION_PROFIT_DECLINE": "Conduct regional performance and cost audit.",
        "FORECAST_PROFIT_RISK": "Take preventive action to avoid next month losses."
    }

    alert["recommendation"] = recommendations.get(
        alert["type"],
...
def run_rule_engine(df, sales_threshold, profit_threshold, discount_threshold):
    ...
    if not valid:
        return [{
            ...
            "message": error,
            "severity": "High",
            "recommendation": "Fix dataset columns"
        }]
    ...
    forecast_alerts = forecast_profit_risk(df, profit_threshold)

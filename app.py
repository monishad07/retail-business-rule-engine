...
if uploaded_file:
    ...
    st.sidebar.subheader("Filters")
    selected_regions = st.sidebar.multiselect("Select Region", sorted(df["Region"].unique()))
    selected_products = st.sidebar.multiselect("Select Product", sorted(df["Product"].unique()))
    ...
    alerts = run_rule_engine(filtered_df, sales_threshold, profit_threshold, discount_threshold)
